...
1. **比 農水省÷変種A が1を超えないこと** — ただし**地方部（圏外率A≥30%）で**見る。
   都市部は操作化そのものが破れているので判定に使えない（下記）。
2. **農水省公表値との相関 r が上がること** — ただし**地方部に限定した r** で見る。
   全国 r は都市-地方のコントラストで稼いでいて（全国 0.271 に対し地方部内 0.158）、
   **純増が都市部に多いソースは地方の穴を1つも埋めなくても r を動かせる**。

**3県だけで判定してはいけない**。47県で測ると結論が逆転した実例がある（下記）。
