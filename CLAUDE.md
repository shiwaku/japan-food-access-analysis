# CLAUDE.md — japan-food-access-analysis

このリポジトリで作業するときの前提・手順・落とし穴をまとめる。次回はまずこれを読むこと。

## このリポジトリは何か

**食料品店まで道路距離500m以上のところに住む人口**を、国勢調査 **125mメッシュ人口** ×
食料品店 POI で推計する。農水省「[食料品アクセス](https://www.maff.go.jp/j/shokusan/eat/access_genjo.html)」の
**食料品アクセス困難人口**は突合の相手であって、推計の対象ではない。

### ★ 推計するのは距離条件だけ（2026-08-28 に確定）

農水省の困難人口は3条件（500m以上 **かつ** 自動車利用困難 **かつ** 65歳以上）だが、
**本リポジトリは第1条件しか推計しない**。自動車利用困難率を外部データで固定できておらず、
公表値との比（0.42〜0.66）が自動車要因と店舗レイヤの穴のどちらに由来するか分離できないため。
年齢条件も外せるように **65歳以上と総人口の両方**を出す。
→ **「困難人口」という語を成果物に使わないこと。** 出すのは「500m圏外人口」。
→ **「自動車利用困難率を固定する」は前提から外れた**。必要になるのは公表値そのものを
   再現しようとする場合だけ（そのときの出所は農水省が使う**家計調査個票**）。

- **人口レイヤ・距離判定・公表値突合がここ**。**店舗レイヤは持たない**（外から受け取る）。
- 店舗レイヤの構築は [shiwaku/japan-food-store-master](https://github.com/shiwaku/japan-food-store-master)。
  2026-08-27 にそちらから検証器（`validate_access_difficulty.py` / `fetch_mesh_population.py`）を
  分離して発足。分離元の該当コミットは #7 / #16 / #20 / #27。
- 姉妹プロジェクト [japan-transit-desert-analysis-125](https://github.com/shiwaku/japan-transit-desert-analysis-125)
  （公共交通空白地域・国交省定義）と同じ構造の農水省版。あちらの `01_prepare_facilities.py` が
  担う施設供給を、こちらでは japan-food-store-master が担う。
- **⚠ 参照するのは `-125`（125mメッシュ版）。`-125` の付かない250m版・`-100` 版ではない。**
  3本は別リポジトリで中身が違う。メッシュ人口のファイル名・PMTiles 生成・ビューワは
  すべて125m版に合わせること。

## リポジトリ構成

```
scripts/
  01_fetch_mesh_population.py      e-Stat T001225 → data/mesh/mesh125_pop.parquet
  04_road_distance.py              ★距離判定の唯一の出所 → data/mesh_road_dist.parquet
  02_validate_access_difficulty.py 圏外人口の集計＋農水省 表5 突合 → output/*.csv
  03_export_pmtiles.py             GeoJSONL → PMTiles（tippecanoe は WSL 側）
docs/
  検証_アクセス困難人口_メッシュ単位.md  一次記録（操作化の判定・カテゴリ感度・誤差）
input/     店舗レイヤの置き場（.gitignore。japan-food-store-master の parquet）
data/      メッシュ人口・境界・農水省表5（.gitignore。全部スクリプトが自動取得）
output/    市区町村別・都道府県別・カテゴリ感度の CSV（追跡する）
           **現行スキーマの2系統6本だけ置く**（`_ATP⑪_47県` = 内部の最良推計、
           `_公開可能_47県` = 地図で公開している推計）。変種Aで測った当時の20本は 2026-08-28 に削除し、
           集計値を docs の付録A に転記した（生データは git 履歴から取る）。
           スキーマ違いを混在させないこと。
```

## 作業ワークフロー（厳守）

- **main へ直接コミット・push しない**。必ずブランチを切って PR を作成しマージする。
  ```
  git checkout -b <type>/<topic>      # feature/ fix/ docs/ 等
  git push -u origin <branch>
  gh pr create --base main --head <branch> --title "..." --body "..."
  gh pr merge <n> --squash --delete-branch
  git checkout main && git pull --ff-only origin main
  ```
- コミットメッセージ末尾に `Claude-Session: <url>` を付ける。

## ★ 判定の原則（これを外すと全部無意味になる）

**件数を足せば圏外率は必ず下がるので、下がったこと自体は判定材料にならない。**
店舗ソースの投入可否は次の2つで見る（`02_validate_access_difficulty.py` が両方出す）:

1. **比 農水省÷本推計 が1を超えないこと**
2. **農水省公表値との相関 r が上がること**

**比の値そのものは推計の精度ではない**（公表値は3条件込みなので、比は「暗黙の自動車利用困難率」）。
使えるのは「1を超えたら論理矛盾」という必要条件のチェックだけ。
店舗ソースを比べるときは `04` を回し直して**同じ道路距離の土俵で**測ること。

**どちらも「地方部＝高齢者密度 < 1,500人/km²（可住地ベース）」に限定して見る**
（1,586市区町村・高齢者 2,251万人）。理由は2つ:

- 都市部は操作化そのものが破れている（密度1,500以上では比>1 が 44〜97%、未満では約1%）
- 全国 r は都市-地方のコントラストで稼いでいて、**純増が都市部に多いソースは地方の穴を
  1つも埋めなくても r を動かせる**

**区分に圏外率を使わないこと。** 圏外率は測る対象そのものなので、店舗レイヤを差し替えると
区分も動いて比較にならない。密度は**店舗レイヤに一切依存しない**（検証器が
`高齢者密度_人per_km2` を出す）。

**3県だけで判定してはいけない**。47県で測ると結論が逆転した実例が2つある:
- 「変種Aは全市区町村で比≤1」→ 全国では102件破れる
- 「生鮮3業種を先に入れろ／⑪ は後回し」→ **地方部では生鮮が悪化・⑪ が中立で逆**

## ★ 農水省の推計方法（2026-08-28 判明・issue #4）

農林水産政策研究所 研究成果報告会資料
[240319_01.pdf](https://www.maff.go.jp/primaff/koho/seminar/2023/attach/pdf/240319_01.pdf)
スライド5に明記されている。

> - 人口のある500mメッシュ単位に、当該及び周辺メッシュの店舗状況から、
>   **最も近い店舗が500m以上である確率**を算出。
> - **この確率に人口を乗じて**、メッシュごとに店舗まで500m以上の人口を推計。

図は「各メッシュにおける500m以上の確率 × 半径500mの円」で、3×3メッシュに
0.939 / 0.828 / 1.000 / 0.385 / **0.028** / 0.955 / 0.598 / 0.259 / 0.989、「メッシュ22内の位置k」。

**判定は二値ではなく確率（面積比）。** ここで比べてきた変種A/C/D は全部二値なので構造が違う。
**店舗のあるメッシュでも確率は0にならない**（図の中心 0.028）ので、千代田区のような
「圏内なのに困難人口」が自然に出る。従来推計の店舗データが**商業統計メッシュ統計＝
メッシュ別件数（座標なし）**なので、店舗位置もメッシュ内で不確実として扱っている、という読み。

| 項目 | 内容 |
|---|---|
| 距離 | **直線500m**（道路距離ではない） |
| 店舗データ | 2020年推計は**2021年スーパー名鑑等**（〜2015年は商業統計メッシュ統計） |
| 業種 | 2020年推計で酒・菓子パン小売業を**除き**ドラッグストアを**追加**（現在の `cat` 4分類と整合） |
| 自動車利用率 | **家計調査個票**（〜2015年は全国消費実態調査） |

**issue #4 で否定された仮説**: 年次のズレ（2021年データなので現在のPOIと近い）・業種範囲。

## ★ 判定は道路距離1本（2026-08-28）

**距離判定の出所は `scripts/04_road_distance.py` だけ。** `02` は店舗レイヤ（`FOOD_STORES`）を
読まず、`04` の parquet を結合するだけ。地図（`03` → `docs/index.html`）も同じ。

かつては二値近似を併走させていたが**両方とも廃止した**。

| 変種 | 操作化 | 47県での比>1 | 廃止理由 |
|---|---|---:|---|
| A | 同一500mメッシュ内の店舗存否 | 78件 | 隣メッシュ10m先の店に救われない実効半径250m程度のマス目判定。距離を測っていない |
| B | 自メッシュ+8近傍メッシュ | 多数 | 3県の時点で不整合 |
| C125 | メッシュ重心から直線500m | 399件 | 道路距離があるなら持つ意味が無い |

変種Aを残していた唯一の理由は「農水省の公表値を地方部で再現できる」ことだったが、
**農水省の推計方法が確率版と判明した時点で二値では原理的に届かない**ことが確定したので落とした
（上記「農水省の推計方法」）。**「変種Aだけが整合する」は二値の中での相対評価にすぎない。**

### 業態は入れ子で回す（S1〜S4）

`04` が**入れ子集合ごとに Dijkstra を回す**（4回＋SO・全国約215秒）。カテゴリ感度も道路距離で出る。

| 集合 | 店舗数 | 圏外65歳以上 | 圏外率 | 前段からの削減 |
|---|---:|---:|---:|---:|
| S1 supermarket のみ | 32,696 | 24,276,339 | 68.8% | — |
| S2 ＋convenience | 92,522 | 16,375,665 | 46.4% | **7,900,674** |
| S3 ＋drugstore | 112,181 | 15,537,729 | 44.0% | 837,936 |
| **S4 ＋fresh_food**（主） | 122,249 | **15,058,980** | **42.7%** | 478,749 |

> 2026-09-20 の**道路距離 m 版**（歩行速度を使わず、店舗のスナップ距離を足す）。
> 時間版（v2 まで）は S1 22,816,223 / S4 13,668,822（38.7%）だった。差は
> アクセスリンクが 5km/h で割引されていた分＋スナップ距離の分。

**コンビニの寄与が圧倒的**（790万人＝全削減の86%）。8.1 でメッシュ存否で測ったときの序列とは
意味が違う（あちらは「同一メッシュに1件でもあるか」なので店舗数の多い業態が有利に出やすい）。

入れ子なので S1 ≥ S2 ≥ S3 ≥ S4 の順に距離が縮む。`04` が検算し、`02` も逆転を警告する。

## ★ 公開するのは「公開可能レイヤ」の推計（2026-09-21）

**地図（`docs/index.html` → PMTiles v4）は Overture Places ＋ 食品営業許可オープンデータの
124,970店で作る。** ATP 基準マスターは自前クロール38チェーンが再配布不可で公開物に載せられないため
（japan-food-store-master の `docs/sources/検証_公開可能マスター_Overture_OSM_許可.md`）。
**ODbL を避けるため OSM も外してある**（`food_store_master_public_noosm.parquet`）。

47県・道路距離での実測は **ATP基準とほぼ同等**なので、公開値としてこれを使う。

| レイヤ | 店舗数 | 地方部 r | 地方部 圏外率 | 地方部 比>1 | 公開 |
|---|---:|---:|---:|---:|---|
| ATP⑪（下表の推計・内部用） | 122,249 | 0.2472 | 56.3% | 21 | 不可 |
| **公開可能（Overture＋許可）** | **124,970** | **0.2490** | **56.5%** | **19** | **可** |

公開可能レイヤの全国値: **圏外65歳以上 15,237,604（43.2%）/ 総人口 47,086,177（37.4%）**、
スーパー圏外 23,997,021（68.0%）、比 0.593、r 0.336、比>1 113件。
ビューワの `STATS`（1,131万 / 876万 / 1,524万）と凡例の店舗数はこの値。

- 内部の最良推計は引き続き ATP⑪（下表）。**2系統が並存する**ので、数字を引くときは
  どちらのレイヤのものか必ず確かめること。
- `serve.py` の店舗レイヤ既定も `food_store_master_public_noosm.parquet` に変えた。

## ★ 47県の推計値（ATP⑪ 122,249店・1,740市区町村・2026-08-28）

農水省 表5 の全国計 9,042,736人・25.6% と突合（1,740/1,740）。

| 判定 | 総人口 | 割合 | 65歳以上 | 割合 |
|---|---:|---:|---:|---:|
| **道路距離500m以上（S4）** | **46,323,468** | **36.7%** | **15,058,980** | **42.7%** |
| うちスーパー（S1）まで500m以上 | — | — | 24,276,339 | 68.8% |

> 2026-09-20 の道路距離 m 版。時間版（〜v2）は 41,333,085（32.8%）/ 13,668,822（38.7%）。

母数 総人口 126,088,473 / 65歳以上 35,314,481人。
**分母は道路距離が定義できるメッシュに限る**（アクセスリンク欠落 1,853メッシュ・
65歳以上 8,450人＝0.024%）。分母をそろえないと率が濁る。

| 領域（高齢者密度・可住地） | 市区町村 | 65歳以上 | 圏外率 | スーパー圏外率 | r | 比>1 |
|---|---:|---:|---:|---:|---:|---:|
| 全国 | 1,740 | 3,531万 | 42.7% | 68.8% | 0.333 | 118 (6.8%) |
| 都市部（≥1,500） | 154 | 1,280万 | 18.6% | 48.7% | 0.731 | **97 (63.0%)** |
| **地方部（<1,500）** | 1,586 | 2,251万 | **56.3%** | **80.2%** | **0.247** | **21 (1.3%)** |

> 道路距離 m 版（2026-09-20）。時間版は 全国 38.7%/0.337/173件、都市部 15.3%/0.707/123件、
> 地方部 52.0%/0.258/50件。比 農水省÷本推計 は 0.662 → **0.600**。

千代田区のように密な市街地では圏外がほぼ消えるのに公表値は数%ある＝**「圏内なのに困難人口」**。
**判定を厳しくしても緩くしても破れは残る**（メッシュ存否78件・道路距離 時間版173件／m版118件・直線399件）。
原因は確率版との構造差（上記）。

**⚠ ATP基準＋許可の投入は全国で測ると悪化している。** 3県では「比≤1 維持・r 0.448→0.462 改善」で
投入が正当化されたが、47県（変種Aで測った時点）では **比>1 が 44→102件に倍増し
r は 0.305→0.271 に悪化**する。純増が都市部に偏っているため。
japan-food-store-master #43 / #44 の判断は測り直しが要る。

**判定**: **地方部の**相対比較・スクリーニングには耐える（比>1 は1.3%）。
**都市部では農水省の公表値を再現できない**（issue #4）。
**絶対人数を1桁目まで信じない**（市区町村単位の店舗由来誤差は概ね ±20〜30%）。

### 次の一手（優先順）

1. **確率版（変種E）を実装して都市部を再現できるか測る**（issue #4）。
   農水省の推計方法は **2026-08-28 に判明済み**（後述「農水省の推計方法」）。
   二値ではなく**確率＝メッシュ内位置についての面積比**なので、変種A/C/D とは構造が違う。
2. ~~自動車利用困難率を外部データで固定する~~ → **前提から外れた**（距離条件だけの推計にしたため）。
3. ~~検証県を47都道府県に拡張~~ → **2026-08-27 に完了**（issue #2）。
4. 店舗レイヤの穴埋めは japan-food-store-master 側の課題。効果は必ずここの検証器で
   **47県・地方部限定の指標**で測る。3県での判定は当てにならない。
   実測済みの結論（issue #46）: **ATP基準への組み替えは改善、許可⑪ は中立、生鮮3業種は悪化**。
   生鮮の破れは離島・極小自治体に集中する（高齢者千人未満の120市区町村で6.7%）。
   **Japan Food Facilities（食品営業許可・届出の統合オープンデータ）は 2026-09-21 に測って不採用**。
   マスタ側から見た再現率（100m以内に届出点があるか）が supermarket 46.1%・convenience 28.5%、
   県別では supermarket 15〜63% とばらつく。補完投入しても地方部で比>1 が 21→26件に増え
   r は 0.2472→0.2470 で改善しない（docs の13節）。**再検討するなら13節を先に読む**。
   **届出の「⑪ 百貨店、総合スーパー」は業態混在**（スーパー49.9%・ドラッグ34.1%）。業種名で cat を振らないこと。

## 再現の要点

```bash
python scripts/01_fetch_mesh_population.py
# 距離判定はここが唯一の出所。入れ子4回の Dijkstra で全国180秒
FOOD_STORES=input/food_store_master_atp_super.parquet python scripts/04_road_distance.py
OUT_SUFFIX=_ATP⑪ python scripts/02_validate_access_difficulty.py 高知県 島根県 宮城県
```

- **`02` は `FOOD_STORES` を使わない。** 店舗レイヤを差し替えたら `04` を回し直すこと。
  `04` の `FOOD_STORES` / `OUT_ROAD` と `02` の `ROAD_DIST` / `OUT_SUFFIX` で実験を分ける。
  **既存 CSV を実験で上書きしないこと**。
- 店舗レイヤに必要な列は **`lat` / `lng` / `cat`** の3つだけ。
  `cat` は `supermarket` / `convenience` / `drugstore` / `fresh_food`。
- 47県で回すときの店舗レイヤは **`food_store_master_atp_super.parquet`（ATP基準＋許可⑪・122,249店）**。
  地方部の実測で最良（issue #46。生鮮3業種は悪化するので入れない）。
- 外部データは全部スクリプトが自動取得・キャッシュ（`data/mesh/` `data/boundary/`
  `data/maff_2020_table05.xlsx`、いずれも .gitignore）。

## 道路距離（scripts/04_road_distance.py）

**算出方法は japan-transit-desert-analysis-125 の `02_calc_transit_desert.py` を踏襲する**が、
**重みは所要時間ではなく道路長 `dist_m`**（2026-09-20 に切り替え）。
店舗を最近傍1道路ノードにスナップ → Multi-source Dijkstra → アクセスリンク L6 でメッシュ距離に変換。
閾値は **500m そのもの**。到達不能は圏外に倒し、アクセスリンクが無いメッシュは落とす。

- **★ 歩行速度・所要時間は一切使わない。** ネットワークの `time_001min` は道路リンクが 3.6km/h、
  **アクセスリンクが 5km/h** で刻まれていて混在しており、時間で測るとメッシュ→道路の区間だけ
  約28%割引になっていた（v2 までの時間版との差: S4 で 17,975 メッシュ・65歳以上 249,550人が
  圏内→圏外）。-125 は時間で測っているので、**この点は踏襲しない**。
- **測っているのは「メッシュ重心 → アクセスリンク → 道路最短経路 → 店舗の最寄りノード → 店舗座標」
  の合計 m。** 店舗側のスナップ距離（直線・中央31〜35m・p99 約130m）もスーパーソースの辺の重みとして
  足している（`SNAP_OFFSET=0` で外せる）。メッシュ側のアクセスリンク長と対称にするため。

- ネットワークはリポジトリ外。**既定はリポジトリの隣の
  `../ksj/ksj-roadcenterline-route-search/01_MakeNetwork/nationwide_walk/`**（`NETWORK_DIR` で変更可）。
  道路リンク 24,045,959 / ノード 18,614,424 / **アクセスリンク L6 23,740,720**。
  入れ子4集合＋SO で全国約215秒。
- **有効ノード 18,441,645 が -125 の記録値と一致するか**を毎回確認すること。ズレたら
  ネットワークか成分フィルタが違う。
- **駅の4象限スナップはしない**（線状障害物が店舗には無いため。-125 のバス停と同じ最近傍1ノード）。
- 出力は `mesh_code` / `dist_S1..S4_m` / `out500m_S1..S4`（**m 単位**。v2 までは `dist_*_min` の
  徒歩分だった）。**主指標は S4（全カテゴリ）**、地図の緑／橙の切り分けは S1（スーパー）。
  ほかに入れ子の外の補助集合 **`SO`（スーパー以外＝convenience+drugstore+fresh_food）** を
  同じ Dijkstra で出す（`dist_SO_m` / `out500m_SO`）。地図の「最寄りコンビニ等まで」専用で、
  `02` の集計には使わない。`min(S1, SO) == S4` を検算する。5集合で全国約4分。
- **分母は道路距離が定義できるメッシュに限る**（`out500m_S4 is not null`）。アクセスリンクが
  無いメッシュは出力に現れないので、分母をそろえないと率が濁る（全国1,853メッシュ・
  65歳以上 8,450人＝0.024%）。
- 店舗は最寄りノードにスナップするが、**スナップ距離（店舗座標→ノードの直線）は距離に足してある**
  （2026-09-20 から。-125 は足していない）。スナップ距離は中央 **35m** / p90 67m / p99 133m /
  最大 956m。同じノードに複数店舗が付いたら最短を採る。

## ビューワ（docs/）

- 作りは **japan-transit-desert-analysis-125** の `docs/index.html` を踏襲。
  住所検索（地理院API）・2D/3D・凡例/集計トグル・ボトムシート・iOS の16px対策まで同じ。
- **判定は道路距離だけ**。判定方式の切り替えボタンは廃止した。
- **区分はタイルに焼いていない**。`out_d`（全カテゴリ500m超）と `out_sm`（スーパー500m超）から
  MapLibre の式で導く（`catCodeExpr()`）ので、色や区分の定義を変えるだけならタイルを
  作り直さなくてよい。**プロパティを増やす／変えるときは作り直しが要る**。
- タイルのプロパティは `out_d` / `out_sm` / `dsm`（最寄りスーパー m・整数）/ `dcv`（最寄りコンビニ等 m・整数）。
  **v3 から m**（v2 までは徒歩分で、ビューワが ×60 していた。今は換算しない）。
  `dcv` は `04` の補助集合 **SO（convenience+drugstore+fresh_food）** の距離で、`03` が
  `04` の parquet を `mesh_code` で結合して焼く（`02` の出力には無い）。
  ポップアップは「最寄りスーパーまで／最寄りコンビニ等まで」の2行（2026-09-20）。
  `dmin`（全カテゴリの最寄り分）は S1 と SO の近い方と同じなので**廃止**。
  `out_a` / `out_c` / `sm` / `cv` / `dg` / `fr` も**廃止した**。
- **集計パネルは総人口が主**（2026-09-21）。年齢で絞るのは本推計の前提ではないので、
  65歳以上は「うち65歳以上」の1行だけにしてある。
- **UI の説明文は削ってある**（2026-09-21）。消したのは集計パネルの赤い注意ボックス・
  「妥当性」・「条件」・「正解値（農水省 表5）」・「直線距離でもメッシュ単位の存否でもない」・
  「Multi-source Dijkstra…」、ポップアップの「道路距離500m判定：圏外」、
  凡例の「※ 条件は距離だけ…『アクセス困難人口』ではない」。
  **地図はどこにも「アクセス困難人口」と書いていない**（表題は「食料品店アクセスマップ」、
  凡例は「最寄りの食料品店まで500m」）ので、断り書きが無くても誤った主張にはならない。
  **ただし成果物に「困難人口」の語を使わない原則は変わらない。**
  農水省の定義との違いは README と `docs/検証_アクセス困難人口_メッシュ単位.md` に書いてある。
- **ポップアップはクリック/タップだけ**（2026-09-21）。ホバーでは出さない。
  ホバーはカーソルを pointer にするだけで、`pinned` フラグは不要になったので消した。
- **スマホの右上は 凡例(12) → 集計(52) → 3D(92) → 店舗(132) → 現在地(172) の縦積み**
  （2026-09-21）。大きさは 52×32px で揃える。**`#map-controls button` に詳細度で負けるので、
  上書きは `#map-controls #view-toggle` のように id を2つ重ねる**こと。
  モバイル幅の確認は**幅390pxの iframe にページを読み込ませて computed style を読む**のが早い
  （ウィンドウのリサイズは効かないことがある）。
- **右上のトグルの ON は黒背景で示す**（`#map-controls button.on`）。3D ボタンは
  ラベルを 3D↔2D で入れ替えていたため**押しても白のまま**で、店舗ボタンだけ黒くなっていた。
  ラベルは「3D」で固定し、状態は背景で示す（黒い「2D」は 3D 表示中を指すことになって読めない）。
- **右上のトグル（3D・店舗）は `#map-controls` にまとめてある**。個別に `right: 308px` と
  直書きしていたら**集計パネルの下に潜って 3D が見えなくなっていた**。位置は
  `--search-w`（住所検索パネル幅）からの算術で置き、大きさも `#map-controls button` で揃える。
  モバイルの上書きは **`#map-controls #view-toggle` と id を2つ重ねる**こと
  （`#map-controls button` は id+要素で詳細度が高く、`#stores-toggle` 単独では負ける）。
- 集計パネルの数値は `STATS` にハードコードしてある。**再集計したら更新すること**。
  **凡例と出典の店舗数（合計 122,249・業態別4つ）もハードコード**。店舗レイヤを差し替えたら
  `select cat, count(*) from '<FOOD_STORES>' group by 1` で出し直して両方直す。
- **`#stats` だけ `backdrop-filter` を外してある。** モバイルで `overflow-y:auto` になり、
  **内部スクロールを持つパネルに backdrop-filter を掛けると実GPUで合成不具合（白化・欠け）**が出る
  （japan-food-store-master の CLAUDE.md 既知の落とし穴）。-125 は掛けているが踏襲しない。
- PMTiles は `-Z4 -z13 --no-tile-size-limit --no-feature-limit --coalesce-densest-as-needed -P`
  で -125 と同じ。2,814,449 ポリゴンで約400MB。**gitignore なので公開用は Cloudflare R2** に置く。
- **公開 PMTiles の置き場は R2 バケット `shi-works` の
  `pmtiles/japan-food-access-analysis/food_access_125m_v4.pmtiles`**（配信 URL は
  `https://shi-works.com/pmtiles/japan-food-access-analysis/…`）。2026-09-20 に初回アップロード
  （`food_access_125m.pmtiles`）、同日 `dmin`→`dsm`/`dcv` で `_v2`、同日 徒歩分→道路距離 m で
  `_v3`、2026-09-21 に店舗レイヤを公開可能版（Overture＋許可 124,970店）へ替えて
  **`_v4` に版替え**。旧版は戻せるように残してある
  （v1: 時間版・dmin、v2: 時間版・dsm/dcv、v3: 道路距離 m・ATP⑪ 122,249店）。
  旧 Xserver（`shiworks2.xsrv.jp`）は 2026-09-06 に R2 へ移行済みなので**使わない**。
  手元からは AWS CLI のプロファイル `r2-shiworks` で入れる（手順・CORS・注意点は
  `C:/Users/yshiw/Documents/xserver-cleanup/R2-STRUCTURE.md`）。
  **差し替えは同じキーに上書きせず、ファイル名の版を上げて（`_v2`, `_v3`…）ビューワの
  `PMTILES_URL` を切り替える**（`03` の `PMTILES_NAME` で出力名を変える）。PMTiles は Range で
  読むので、上書きすると Cloudflare のキャッシュに新旧の断片が混ざって壊れる。パージの手順は
  xserver-cleanup にも無い。旧版は参照が切れたのを確認してから消す（消さなくてもよい）。
  隣の `pmtiles/food-access/food_desert_125m.pmtiles` は japan-mobility-ease-diagnosis の別物。触らない。
- **地図の名前は「食料品店アクセスマップ」**。農水省の「食料品アクセスマップ」とは別物
  （こちらは店舗までの距離だけを測る）。出典欄で農水省の製品名を書くときだけ元の名前を使う。
- **`serve.py` は `ThreadingHTTPServer` でないと使い物にならない。** PMTiles は並列に大量の
  Range リクエストを投げるうえブラウザが keep-alive で接続を掴むので、シングルスレッドだと
  後続が全部ブロックされて地図が固まる。
- ローカル確認は `python serve.py 8080` → http://localhost:8080/docs/
  （PMTiles は Range リクエストが要るので `python -m http.server` では不可）。
- **店舗は z13 以上でピン（アイコン）、z10〜13 は丸点**（2026-09-21）。アイコンは
  japan-food-store-master の viewer と同じ
  [custom-smartmap-sprite](https://github.com/shiwaku/custom-smartmap-sprite)（MIT / Geolonia）。
  **MapLibre の複数スプライトは、接頭辞なしで引けるのが id `default` だけ**なので、
  地理院スタイルを `default` に据え、追加分は `smartmap:<アイコン名>` で引く。
  読み込み後に `map.setSprite()` で配列に差し替える（`addFoodSprite()`）。
  **凡例の色はアイコンの色と手動同期**（`STORE_CAT` の color）。アイコン側を変えたら直すこと。
- **`docs/index.html` のスクリプトは module ではない**。`<script>` のままなので
  **トップレベル `await` は構文エラーになり、スクリプト全体が動かなくなる**（地図が
  「読み込んでいます」から進まない）。スタイルを fetch して書き換える実装で踏んだ。
  非同期が要るときは関数の中に閉じ込めること。`node --check` で確かめるなら **`.mjs` ではなく
  `.js` として**チェックする（.mjs だと module 扱いでトップレベル await が通ってしまう）。
- **店舗の点は公開ページにも出る**（2026-09-21）。店舗レイヤを全行再配布可の
  Overture＋食品営業許可（124,970店）に替えたため。配信は**別タイル**
  `https://shi-works.com/pmtiles/japan-food-store-master/food_store_master_public_v1.pmtiles`
  （レイヤ名 `stores` / 属性 `cat` `name` `src` `sources` `license`）で、
  メッシュのタイルには焼いていない。ビューワの `STORES_URL` / `STORES_COUNT` で指す。
  生成は japan-food-store-master の `scripts/build_public_master_pmtiles.sh`。
  **ポップアップに `sources` を出す**（CC BY 系は帰属表示が条件。無い行は Overture 表記）。
  ローカルで `serve.py` が動いていて `/dev/stores.geojson` が読めるときは**そちらを優先**する
  （既定 `input/food_store_master_public_noosm.parquet`）。別の店舗レイヤを見たいときは
  `FOOD_STORES=input/xxx.parquet python serve.py 8080`。
  ⚠ 2026-09-20 まではローカル専用だった（自前クロール由来が38%あり再配布不可だったため）。

## 落とし穴・環境メモ

- **e-Stat メッシュ統計の列番号**: `…019` が65歳以上、`…022` は**75歳以上**。取り違えると分母が半分になる
  （仙台市 65+ = 255,567 が 126,553 になる）。**125m（T001225）と 500m（T001141）で並びは同一**。
- **e-Stat 統計GIS は appId 不要**でダウンロードできる。125mメッシュ人口は
  `statmap-search/data?statsId=T001225&code=<1次メッシュ>&downloadType=2`。境界も同様。
  e-Stat **API** の appId はユーザー保有で、リポジトリには無い。
- **125mメッシュは秘匿が桁違いに多い**。全国 2,820,831 メッシュ中 **1,017,247 メッシュ**が
  65歳以上 `*`（秘匿）。`HTKSYORI=2` の行は `HTKSAKI` メッシュに合算済みなので `*` は0扱いで
  総計は保たれるが、人口が合算先メッシュの位置に寄る。県計・市区町村計の目減りは必ず確認すること。
- **DuckDB の `/` は DOUBLE を返し `::int` が四捨五入する**。メッシュ添字は必ず `//`（整数除算）を使う
  （`(m-1)/2` で m=2 が 0.5→1 に丸められ別メッシュと衝突した）。
- **DuckDB spheroid バグ**: この環境では `ST_Distance_Spheroid` / `ST_DWithin_Spheroid` が `-nan` を返す。
  距離は等距円筒近似（緯度補正した平面距離）で代替する。
- **`ST_Read` の戻り型は `GEOMETRY('EPSG:4612')`** で、そのままでは rtree インデックスが作れない。
  `geom::GEOMETRY` で素の型に落とす。
- **e-Stat 境界 shapefile の DBF は CP932**。`CITY_NAME`/`PREF_NAME` を読むと DuckDB が unicode エラーを
  出す。ASCII のコード列（`PREF`,`CITY`）のみ読み、名称は農水省の表5から引く。
- **政令指定都市**: 農水省 表5 は市単位（仙台市=04100）、境界データは区単位（04101…）。
  3桁目が `1` のコードを `XX100` に寄せる。
- **区コードを市へ寄せる規則を自前で書かないこと**。`XX1YY → XX100` は**間違い**で、
  農水省 表5 は東京23区を個別コード（13101〜13123）で持つので寄せてはいけないし、
  政令市の市コードも 100 とは限らない（横浜 14100・**川崎 14130**・相模原 14150・浜松 22130・
  堺 27140・福岡 40130）。川崎区(14131) を 14100 に寄せると横浜市に合算される。
  **表5 を正とし、表5 に無いコードだけを「同一県内で自分以下・末尾0 の最大の表5コード」へ寄せる**。
  3県には仙台市しか無かったのでこのバグは 2026-08-27 の47県拡張まで露見しなかった
  （東京23区の65歳以上 2,028,298人が丸ごと欠落していた）。
- **メッシュ人口は既定で `input/2020_pop_census_mesh125.parquet` を読む**
  （姉妹リポジトリ japan-transit-desert-analysis-125 と同じ流儀・同じファイル名）。
  全国が1本に入っているので県名の指定は不要。e-Stat からの取得はファイルが無いときのフォールバックで、
  全国だと1次メッシュ151個・数十分かかる。
- **`/c/Users/...` のような Git Bash 形式のパスは Windows の Python が解決できない**。
  `os.path.exists` が黙って False を返すので「入力が無い」で落ちる。`04` の `NETWORK_DIR` が
  これで踏んだ。既定はリポジトリからの相対パスにし、`/c/` → `C:/` の正規化も入れてある。
- **3県だけで回すと都市部（密度≥1,500）が1件も無い**。領域別集計の `sum` が NULL になって
  フォーマットで落ちた。片側が空になるケースを常に想定すること。
- tippecanoe は **WSL 側にしかない**（Windows の PATH には無い）。`03` は「PATH に無い」と言って
  終わるので、`wsl -e bash -lc "cd /mnt/c/... && bash output/make_pmtiles.sh"` で回す。
- Windows の Git Bash でコンソールが文字化けするときは `PYTHONIOENCODING=utf-8` を付ける
  （ファイル出力は影響を受けない）。
